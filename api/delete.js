...
const { findUser, isAdmin, getMessages, saveMessages, pushSystem, vnDate, send, body, cors } = require('./_store');
...
module.exports = async (req, res) => {
  ...
  if (action === 'byDate') {
    ...
    if (!deleted) return send(res, 200, { ok: true, deleted: 0 });
    await saveMessages(all);
    await pushSystem(`🔔 Admin ${me.nick || me.username} đã xóa ${deleted} tin ngày ${date}`);
    return send(res, 200, { ok: true, deleted });
  }
